...
char BLE_Device_Name[] = "ESP32TTGO";
...
Serial.println(BLYNK_ASYNC_ESP32_BT_WF_VERSION);
  ...
  Blynk_BLE.begin(auth);
#else
    USE_BLE = false; //usar wifi y no ble
    Serial.println(F("GPIO14 HIGH, Use WiFi"));
      ...
      Blynk.setConfigPortal("ESP32", "TestPortalPass");
      ...
      //                           IPAddress(4, 4, 4, 4), IPAddress(8, 8, 8, 8));
        ...
        Serial.print(F("Personalización en la visualización de la data"));
      #endif
      
      #if USING_CUSTOMS_HEAD_ELEMENT
        Blynk.setCustomsHeadElement("<style>html{filter: invert(10%);}</style>");
        Serial.print(F("Estilos-Encabezado del HTML"));

      #endif

      #if USING_CORS_FEATURE  
        Blynk.setCORSHeader("Your Access-Control-Allow-Origin"); //para permitir o restringir el acceso a recursos 
        Serial.print(F("Permitir o restringir el acceso a recursos "));
